rsconnect::setAccountInfo(name = "josearenas", token = "B24EF7DD0297ABF2441663F8FAE38E43", secret = "YU2wPT4x92YtDhMhJWAscRMFtZ9LHfvYR3DmBNBU")
...
ui <- dashboardPage(
    ...
    dashboardSidebar(
        sidebarMenu(
            ...
            menuItem("???? Replicaci??n", tabName = "replication", icon = icon("repeat"))
        )
    ),
    dashboardBody(
        tags$head(
      ...
      "))
        ...
        tabItems(
            tabItem(
                ...
                fluidRow(
                    box(
                        title = "??????? Configuraci??n del Experimento", status = "primary", solidHeader = TRUE,
                        width = 12, collapsible = TRUE,
                        fluidRow(
                            column(
                                3,
                            ...
                            column(
                                ...
                                conditionalPanel(
                                    ...
                                    div(
                                        ...
                                        "Se analizar??n: J??venes (20-40), Adultos (40-60), Veteranos (+60)"
                                    )
                                )
                            ),
                            column(
                                3,
                                h5("???? M??ltiples Variables Dependientes"),
                                checkboxInput("multiple_variables", "Probar m??ltiples variables", value = FALSE),
                                conditionalPanel(
                                    condition = "input.multiple_variables",
                                    sliderInput("n_dependent_vars", "N??mero de variables:",
                    ...
                    fluidRow(
                        ...
                        column(
                            ...
                            div(
                                ...
                                div(style = "text-align: center; font-size: 16px;", "Proporci??n < 0.05")
                            )
                        )
                    )
                ),
                fluidRow(
                    box(
                        title = "???? Distribuci??n Te??rica del T-Statistic",
                        status = "primary", solidHeader = TRUE, width = 12,
                        div(
                            style = "margin-bottom: 10px;",
                            h5("???? F??rmula: t-statistic = ?? / error est??ndar de ??"),
                ...
                conditionalPanel(
                    ...
                    fluidRow(
                        box(
                            title = "???? Resultados de la Simulaci??n Actual",
                ...
                fluidRow(
                    box(
                        ...
                        fluidRow(
                            column(
                                4,
                                h5("?????? Configuraci??n del Estudio:"),
                                numericInput("n_replications", "???? N??mero de Estudios:",
                                ...
                                h5("???? Opciones Avanzadas (selecciona m??ximo una):"),
                                checkboxInput("replication_analyze_by_age", "Efectos heterog??neos por grupo etario", value = FALSE),
                                checkboxInput("replication_multiple_variables", "M??ltiples variables dependientes", value = FALSE),
                                conditionalPanel(
                                    condition = "input.replication_multiple_variables",
                                    sliderInput("replication_n_dependent_vars", "N??mero de variables:",
                                        min = 2, max = 5, value = 3, step = 1
                                    )
                                ),
...
server <- function(input, output, session) {
    ...
    calculate_statistics <- function(data, var_index = 1, group_name = "Todos") {
        ...
        if (nrow(treatment_data) < 2 || nrow(control_data) < 2) {
            return(NULL)
        }
    ...
    run_analysis <- function(sample_data, replace_current = TRUE) {
        ...
        groups_to_analyze <- if (input$analyze_by_age) {
            list("J??venes (20-40)" = "jovenes", "Adultos (40-60)" = "adultos", "Veteranos (+60)" = "veteranos")
        } else {
            list("Todos" = "todos")
        }
    ...
    output$sample_size_text <- renderText({
        paste("????", input$sample_size, "participantes")
    })
    ...
    output$current_context <- renderText({
        analyses_text <- if (input$multiple_variables && input$analyze_by_age) {
            "M??ltiples variables ?? m??ltiples grupos"
    ...
    output$total_tests <- renderText({
        nrow(values$current_results)
    })
    output$p_values_under_05 <- renderText({
        sum(values$current_results$P_Value < 0.05, na.rm = TRUE)
    })
    output$proportion_under_05 <- renderText({
        if (nrow(values$current_results) == 0) {
            return("0%")
        }
    ...
    output$has_sample <- reactive({
        !is.null(values$current_sample)
    })
    outputOptions(output, "has_sample", suspendWhenHidden = FALSE)
    output$has_results <- reactive({
        nrow(values$current_results) > 0
    })
    outputOptions(output, "has_results", suspendWhenHidden = FALSE)
    output$has_replication_results <- reactive({
        !is.null(values$replication_results)
    })
            ...
            mutate(
                ...
                Plot_Color = {
                    ...
                    if (input$analyze_by_age) {
                        case_when(
                            Grupo_Etario == "J??venes (20-40)" ~ alpha(base_color, 0.6),
    ...
    output$theoretical_plot <- renderPlotly({
        ...
        p <- ggplot(theoretical_data, aes(x = x, y = y)) +
            ...
            labs(title = "Distribuci??n Te??rica del T-Statistic", x = "T-Statistic", y = "Densidad de Probabilidad") +
            theme_minimal() +
        ...
        p <- p +
            annotate("text",
                x = critical_value + 0.3, y = max(theoretical_data$y) * 0.9,
                label = paste("Cr??tico\nsi |t| >", critical_value),
    ...
    output$current_analysis_info <- renderTable(
        {
            ...
            data.frame(
                Aspecto = c("???? Tipo de An??lisis", "???? Participantes", "???? Pruebas", "???? Efecto Real"),
                Detalle = c(
                    analysis_type, nrow(values$current_sample), nrow(values$current_results),
    ...
    output$main_results <- renderTable(
        {
            ...
            data.frame(
                Estad??stica <- c("???? Beta (Efecto)", "???? Error Est??ndar", "???? T-Statistic", "???? P-valor"),
    ...
    output$detailed_results_plot <- renderPlotly({
        ...
        p <- ggplot(plot_data, aes(x = Experiment_ID, y = P_Value_Numeric)) +
            geom_point(
                aes(
                    ...
                    text = paste("An??lisis:", Combined_Label, "<br>P-valor:", round(P_Value_Numeric, 4))
                ),
                alpha = 0.9
            ) +
        ...
        p <- p + annotate("text",
            x = max(plot_data$Experiment_ID, na.rm = TRUE) * 0.7, y = 0.08,
            label = "L??nea p = 0.05",
    ...
    output$analysis_summary <- renderText({
        ...
        paste(
            "Simulaci??n actual:", n_vars, "variables ??", n_groups, "grupos =",
            nrow(values$current_results), "an??lisis totales.", n_under_05, "con p < 0.05."
        )
    })
    ...
    output$current_results_table <- DT::renderDataTable({
        ...
        DT::datatable(
            ...
            options = list(
                ...
                columnDefs = list(
                    list(
                        targets = which(colnames(display_data) == "P_Value") - 1,
    ...
    output$rejection_rate_evolution <- renderPlotly({
        ...
        caption_text <- "La l??nea roja horizontal indica el valor de 5% del eje y. "

        if (input$replication_multiple_variables && input$replication_analyze_by_age) {
            caption_text <- paste0(
                caption_text,
                "L??nea azul: an??lisis con todos los datos. ",
                "L??nea verde discontinua: proporci??n de estudios donde al menos una variable es significativa. ",
                "L??nea naranja discontinua: proporci??n de estudios donde al menos un subgrupo etario es significativo."
            )
        } else if (input$replication_multiple_variables) {
            caption_text <- paste0(
                caption_text,
                "L??nea azul: an??lisis con todos los datos usando una variable. ",
                "L??nea verde discontinua: proporci??n de estudios donde al menos una de las ",
        ...
        p <- ggplot(data, aes(x = replication)) +
            ...
            labs(
                title = "Evoluci??n de la Tasa de p-valor < 0.05",
                x = "N??mero de Estudios Acumulados",
                y = "Proporci??n de p-valores < 0.05",
                color = "An??lisis",
    ...
    output$n_studies <- renderText({
        ifelse(is.null(values$replication_results), "0", nrow(values$replication_results))
    })
}
