              ...
              fluidRow(
                box(
                  ...
                  div(
                    ...
                    p("??Bienvenido al laboratorio! Est??s investigando la efectividad de ",
...
server <- function(input, output, session) {
  ...
  output$detailed_results_plot <- renderPlotly({
    ...
    p <- ggplot(plot_data, aes(x = Experiment_ID, y = P_Value_Numeric)) +
      ...
      labs(title = "???? Todos los An??lisis de TralaleroTralaLex",
  ...
  output$analysis_summary <- renderText({
    ...
    paste("Simulaci??n actual:", n_vars, "variables ??", n_groups, "grupos =",
  ...
  output$rejection_rate_evolution <- renderPlotly({
    ...
    p <- ggplot(data, aes(x = replication)) +
      ...
      labs(title = "Evoluci??n de la Tasa de p-valor < 0.05",
